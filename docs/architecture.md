...
- `GmMapView` — interactive token map with read-only stat-block hover tooltips and health bars (name/type/characteristics/wounds/strain/soak/defense as plain text; `pointerEvents: 'none'` unconditionally, no click-lock/pin mode, no ± combat controls — `useEncounterCombatControls` is not called from this file). `onTokenClick` fires on a genuine tap (not a drag) for the Encounter Deck to focus the corresponding card. `handleRemoveToken` (right-click context-menu remove) does its own minimal, non-optimistic `combat_encounters` write for the initiative-slot cascade-delete. Task 6 adds `const [combatCheckState, setCombatCheckState] = useState<{ entityId: string, weaponIndex: number, targetId: string } | null>(null)`, owned here rather than lifted to `GmShell.tsx` (unlike `dossierEntityId`) because `CombatCheckOverlay`'s `.hud-quick-drawer` CSS (`position:absolute; top:0; left:0; bottom:0`) needs a full-height positioned ancestor — it mounts as a `position:relative` MAP-AREA sibling of `EncounterDeck`/`EncounterDossier`, not nested inside `CheckConsole`'s narrow column. `EncounterDossier`'s `onOpenCombatCheck` prop is wired here to `if (dossierEntityId) setCombatCheckState({ entityId: dossierEntityId, weaponIndex, targetId })` — `dossierEntityId` (already a `GmMapView` prop from `GmShell`) doubles as the attacking entity id, since it's exactly the entity whose dossier was open when the GM clicked "OPEN COMBAT CHECK". When `combatCheckState` is set, an IIFE re-resolves the attacker via `buildRoster(encounterProp, tokens, advTokenImages, vehTokenImages)`, early-returning `null` if the resolved entry isn't `kind === 'adversary'` (defensive — `CheckConsole` already disables the button for vehicles) or the weapon index doesn't resolve. Adapts the attacker via `adaptAdversaryForCombatCheck(attackerAdv, campaignId)` (Character/CharacterWeapon/CharacterSkill/RefWeapon/RefSkill stubs), derives `gmTargets` (PC stubs via `charactersToAdversaryStubs` + allied-NPC adversaries from `encounterProp`, same derivation as `CheckConsole`'s `targetOptions`), and `initialAttackType` via `isMeleeSkill(weaponSkillKey(weapon))`. Mounts `CombatCheckOverlay` with `gmOverrides={{ isGmMode: true, gmTargets, gmAlignment: attackerEntry.alignment }}`; `onRoll` calls `logRoll` with `characterId: null, isDM: true, hidden: false` (hardcoded, no UI path to change it — mirrors the deleted `EncounterAdversaryPanel`'s `handleAdvRoll` pattern, minus its `hidden: !adv.revealed` branch, since this plan's rule is "combat/skill checks from this tab are always public").
