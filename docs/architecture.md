...
- `EncounterDeck` (`src/components/gm/EncounterDeck.tsx`) — **shell only, in progress**: a `position:absolute` (never `fixed`) pull-up card drawer docked to the bottom of the GM map panel, rendered as a sibling of `MapToolsRadial` inside `GmMapView`'s map-area div when `isStagingTab === true`. Collapsed state is a centered handle bar (`Z.deck`) showing live enemy/friendly counts as red/green badges — enemy/friendly split for adversaries is derived from `encounter.initiative_slots[].alignment` (matched by `adversaryInstanceId`; `AdversaryInstance` itself has no `alignment` field), while `VehicleInstance.alignment` is read directly. Expanded body (`Z.deckExpanded`) is GSAP height-tweened (0 ↔ 236px, `power3.out`/`power2.in`) rather than `display`-toggled, and calls `onMapAreaResize?.()` in `onComplete` so the map canvas can recentre once the drawer finishes animating. Body contains a search input that toggles the empty-state label between "Encounter roster" and "Library results — click ADD to place"; the roster rail / library-results content, and all add/bench/deploy/remove/hidden lifecycle actions, are not yet built (placeholder div only) — deferred to a later task in the same plan. Already wired to `useEncounterCombatControls` (adjust wounds/strain/group-size/hull/system-strain) and `useAdversaryTokenImages`/`useVehicleTokenImages`, but none of those are consumed by the placeholder body yet. **Temporary stub mount**: `GmMapView.tsx` currently mounts this with a local `deckOpenLocal` state and no-op `setEncounter`/`saveEncounter`/`updateTokenWoundPct`/`markPending`/`clearPending`/`stagingAddToEncounter` props, purely to prove the shell renders/animates in isolation — a later task replaces this with real props threaded from `GmShell`/`useGmSession`.
