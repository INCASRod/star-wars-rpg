...
> Updated 2026-07-13. Read from codebase; update when structure changes.
...
- Optimistic: every adjustment updates the caller's encounter state synchronously via `setEncounter`'s functional form, reading the live value from `prev` rather than the closed-over instance — this is what fixed the original click-lag bug where two rapid clicks sharing the same stale `adv`/`vehicle` reference would both compute from the same pre-click baseline and lose a delta
...
- **Known issue**: rapid clicks never lose deltas (the optimistic `setEncounter` fix above holds), but the debounced write in `scheduleWrite` can occasionally fire several seconds late with stale content, causing the displayed/persisted value to visibly revert to a pre-burst number for 1-2+ seconds before self-correcting. Root cause not yet found — needs further investigation in `scheduleWrite`'s timer lifecycle.
