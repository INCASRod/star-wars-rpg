...
- `ForcePanel` / `HudForceTab` — Force powers panel with two tabs: **Powers** (power cards + purchase UI) and **Committed** (shows active force die commitments from `characters.force_commitments`; cancel button decrements `force_rating_committed`). `HudForceTab` owns the `handleCancelCommit` write handler. When `canGainForceRating` is true (eligible via a Force-sensitive spec, Force Rating still 0, not yet purchased), the Powers tab renders a "Gain Force Rating 1 — 10 XP" CTA instead of the power list/browse button, calling `onPurchaseForceRating` (`handlePurchaseForceRating` in `useCharacterData.ts`). Desktop's `HudLeftRail` Force nav tab is now gated on `canAccessForceTab` (`isForceUser || isEligibleForForceRating(...)`, computed in `PlayerHUDDesktop.tsx`) rather than `isForceUser` alone, so eligible-but-unpurchased characters can still reach the tab to buy in; the Force Check quick-action button remains gated on `isForceUser` (Force Rating ≥ 1) only. A dismissible "Force Sensitivity Awakens" prompt (inlined in `HudModalsOverlay.tsx`, driven by `pendingForceRatingOffer` state in `useCharacterData.ts`) offers the same purchase right after buying a Force-sensitive specialization; "Later" does not revert the specialization purchase. Mobile (`MobileForceScreen`/`MobileRunner`) does not yet implement this gating — tracked as a follow-up.
...
| `forceUtils.ts` | Force rating availability helpers (`isForceUserSensitive`, `getAvailableForceRating`), Morality helpers |
