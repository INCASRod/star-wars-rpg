...
| `/character/[id]/talents` | `talents/page.tsx` | Full-screen talent surface — vertical rail of destinations (one per owned spec + "Signature Abilities"), main content area shows exactly one at a time (`contentView: 'tree' \| 'sig'` local state, Prompt 7b) rather than the tree with the sig section always scrolled below it; reads `?gm=1` and `?spec=` query params; replaced `HudTalentTreeModal` (retired). Switching to the sig destination does not change `activeSpecKey` — sig content stays tied to whichever spec was last selected on the tree. Data comes from `useTalentSurfaceData`, not `useCharacterData` (Prompt 7a) — a lean, route-scoped hook so this route doesn't refetch the full character sheet. Mega-header (career line → title → ghost watermark → meta row) plays a once-per-route-load GSAP stagger reveal on first paint (Prompt 9, guarded by a `useRef` flag keyed off `shellReady`, not re-fired by later re-renders); the tree's own tier-by-tier entrance (`runTreeEntrance`, `buildTalentTree.ts`) is a separate concern owned by `TalentTree.tsx` and re-fires on every rail spec-switch. `BuySpecButton`'s `onBuy` awaits `handleBuySpecialization` and, on success, renders `<PurchaseCeremony>` announcing the spec + any newly-granted career skills (see that component's own entry). |
...
- `PurchaseCeremony` (`src/components/character/PurchaseCeremony.tsx`, Prompt 9) — shared portal (`document.body`, `Z.hudSupreme`) for two moments: the talent-purchase ceremony (`TalentCeremonyPayload`, `kind: 'talent'`) and the spec-purchase celebration (`SpecCeremonyPayload`, `kind: 'spec'`, lists newly-granted career skills). Renders only AFTER the real DB write has already resolved — callers (`TalentTree.tsx`'s `dossierAction.onConfirm`, `SigNodeGrid`'s equivalent in `TalentSurface.tsx`, `talents/page.tsx`'s `BuySpecButton` `onBuy`) `await` the mutation and only construct a payload on a truthy/non-throwing result, so the animation never gates or masks the write. The talent ceremony's card is a portalled replica (not the real plaque) that animates from a caller-captured `sourceRect` to centre stage and back — by the time it fades out, the real plaque underneath is already re-rendered in its owned state (the mutation's own optimistic update fired well before this component even mounts), so no clone persists. Skippable by clicking the backdrop (kills the GSAP timeline, same `touched[]`/`clearProps` convention as `runEnergyTrace`/`runTreeEntrance`). Under `usePrefersReducedMotion`, renders the same final content statically for a fixed (talent) or skill-count-scaled (spec, up to 4s) hold instead of animating — information is never skipped, only the motion is. DEDI (dedication) talent purchases deliberately do not trigger this — `TalentSurface.tsx`'s `onPurchase` wrapper returns `undefined` for that branch on purpose (it hands off to `DedicationModal` instead), and `TalentTree.tsx` treats any falsy purchase result as "no ceremony" with no DEDI-specific check needed on that side.
