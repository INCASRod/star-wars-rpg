...
export function useQuartermaster(
  ...
  const loadItems = useCallback(async (qmId: string) => {
    ...
    const [wRes, aRes, gRes] = await Promise.all([
      weaponKeys.length ? supabase.from('ref_weapons').select('key,name,rarity,encumbrance,price,damage,damage_add,crit,description,skill_key,range_value,hard_points,qualities,categories').in('key', weaponKeys) : Promise.resolve({ data: [] }),
      armorKeys.length  ? supabase.from('ref_armor').select('key,name,rarity,encumbrance,price,soak,soak_bonus,defense,encumbrance_bonus,description,categories').in('key', armorKeys)   : Promise.resolve({ data: [] }),
      gearKeys.length   ? supabase.from('ref_gear').select('key,name,rarity,encumbrance,price,encumbrance_bonus,description,categories').in('key', gearKeys)            : Promise.resolve({ data: [] }),
    ])

    type WRow = { key: string; name: string; rarity: number; encumbrance: number; price: number; damage: number; damage_add: number | null; crit: number; description: string | null; skill_key: string | null; range_value: string | null; hard_points: number | null; qualities: WeaponQuality[] | null; categories: string[] | null }
    type ARow = { key: string; name: string; rarity: number; encumbrance: number; price: number; soak: number; soak_bonus: number | null; defense: number; encumbrance_bonus: number | null; description: string | null; categories: string[] | null }
    type GRow = { key: string; name: string; rarity: number; encumbrance: number; price: number; encumbrance_bonus: number | null; description: string | null; categories: string[] | null }
    ...
    const buys: QmBuyRow[] = items.flatMap((qi): QmBuyRow[] => {
      if (qi.item_type === 'weapon') {
        ...
        return r ? [{ qmItem: qi, name: r.name, rarity: r.rarity, encumbrance: r.encumbrance, price: r.price, description: r.description ?? undefined, damage: r.damage, damage_add: r.damage_add, crit: r.crit, skill_key: r.skill_key ?? undefined, range_value: r.range_value ?? undefined, hard_points: r.hard_points ?? undefined, qualities: r.qualities ?? undefined, categories: r.categories ?? undefined }] : []
      }
      if (qi.item_type === 'armor') {
        const r = aMap[qi.item_key]
        return r ? [{ qmItem: qi, name: r.name, rarity: r.rarity, encumbrance: r.encumbrance, price: r.price, description: r.description ?? undefined, soak: r.soak, soak_bonus: r.soak_bonus, defense: r.defense, encumbrance_bonus: r.encumbrance_bonus, categories: r.categories ?? undefined }] : []
      }
      const r = gMap[qi.item_key]
      return r ? [{ qmItem: qi, name: r.name, rarity: r.rarity, encumbrance: r.encumbrance, price: r.price, description: r.description ?? undefined, encumbrance_bonus: r.encumbrance_bonus, categories: r.categories ?? undefined }] : []
    })
