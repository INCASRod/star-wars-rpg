...
function MapForgeInner() {
  ...
  function AssetThumbs({ list, setList, primaryLabel }: {
    ...
    return (
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.375rem', marginBottom: '0.625rem' }}>
        {list.map((a, i) => (
          <div key={a.id} style={{ position: 'relative', width: '4.5rem', height: '4.5rem', borderRadius: RADIUS.md, overflow: 'hidden', border: `1px solid ${i === 0 && primaryLabel ? 'rgba(200,170,80,0.55)' : BORDER}`, flexShrink: 0 }}>
            ...
            {!a.url && (
              <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ width: '1rem', height: '1rem', borderRadius: RADIUS.full, border: `2px solid ${HUD.gold}`, borderTopColor: 'transparent', animation: 'mf-spin 0.8s linear infinite' }} />
              </div>
            )}
            {i === 0 && primaryLabel && a.url && (
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'rgba(200,170,80,0.75)', fontFamily: FONT_BODY, fontSize: 'var(--text-overline)', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: BG, textAlign: 'center', padding: '0.125rem 0' }}>
                {primaryLabel}
              </div>
            )}
            <button
              ...
              title="Remove"
              style={{ position: 'absolute', top: 2, right: 2, width: '1rem', height: '1rem', borderRadius: RADIUS.full, background: 'rgba(0,0,0,0.75)', border: 'none', cursor: 'pointer', color: TEXT, fontSize: 'var(--text-overline)', display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 }}
            >✕</button>
          </div>
        ))}
      </div>
  ...
  return (
    <div style={{ height: '100dvh', overflow: 'hidden', background: BG, display: 'flex', flexDirection: 'column', color: TEXT }}>
      ...
      {/* ── Header bar ──────────────────────────────────────── */}
      <div style={{ flexShrink: 0, height: '3.25rem', display: 'flex', alignItems: 'center', gap: 'var(--space-4)', padding: '0 var(--space-5)', background: PANEL_BG, borderBottom: `1px solid ${BORDER}`, backdropFilter: 'blur(12px)' }}>
        <button onClick={() => router.push(`/gm?campaign=${campaignId}&tab=staging`)} style={btnDim}>
          ← Back to Staging
        </button>
        <div style={{ width: 1, height: '1.375rem', background: BORDER }} />
        <span style={{ fontFamily: FONT_BODY, fontSize: FS_H4, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: HUD.gold }}>
          Map Forge
        </span>
        {editModeActive && (
          <span style={{ fontFamily: FONT_BODY, fontSize: FS_CAPTION, color: DIM, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
            · Edit Mode
          </span>
        )}
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
          ...
          {campaignId && (
            <span style={{ fontFamily: FONT_BODY, fontSize: FS_CAPTION, color: DIM }}>Campaign: {campaignId.slice(0, 8)}…</span>
          )}
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        {/* ── Left panel ──────────────────────────────────── */}
        <div style={{ width: '22.5rem', flexShrink: 0, display: 'flex', flexDirection: 'column', borderRight: `1px solid ${BORDER}`, background: PANEL_BG, overflowY: 'auto' }}>

          {editModeActive ? (
            /* ═══════════════ EDIT MODE PANEL ═══════════════ */
            <>
              {/* Edit instruction */}
              <div style={{ padding: '1.25rem 1.125rem', borderBottom: `1px solid ${BORDER}` }}>
                <div style={{ fontFamily: FONT_BODY, fontSize: FS_OVERLINE, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(200,170,80,0.5)', marginBottom: '0.625rem' }}>
                  Edit Instruction
                </div>
                <textarea
                  ref={promptRef}
                ...
                />
                <div style={{ fontFamily: FONT_BODY, fontSize: FS_CAPTION, color: DIM, marginTop: '0.3125rem' }}>Ctrl + Enter to apply</div>
              </div>

              {/* Reference images to incorporate */}
              <div style={{ padding: '0.875rem 1.125rem 1.125rem', borderBottom: `1px solid ${BORDER}` }}>
                <div style={{ fontFamily: FONT_BODY, fontSize: FS_OVERLINE, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(200,170,80,0.5)', marginBottom: '0.5rem' }}>
                  Reference Images
                </div>
                <div style={{ fontFamily: FONT_BODY, fontSize: FS_CAPTION, color: DIM, marginBottom: '0.625rem', lineHeight: 1.5 }}>
                ...
                <button
                  ...
                  style={{ width: '100%', padding: '0.625rem', border: `1px dashed ${editRefUploading ? BORDER : BORDER_HI}`, borderRadius: RADIUS.md, background: 'transparent', cursor: editRefUploading ? 'wait' : 'pointer', fontFamily: FONT_BODY, fontSize: FS_CAPTION, color: editRefUploading ? DIM : TEXT, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}
                >
                  {editRefUploading
                    ? <><div style={{ width: '0.75rem', height: '0.75rem', borderRadius: RADIUS.full, border: `1.5px solid ${HUD.gold}`, borderTopColor: 'transparent', animation: 'mf-spin 0.8s linear infinite' }} />Uploading…</>
            ...
            <>
              ...
              <div style={{ padding: '1.25rem 1.125rem', borderBottom: `1px solid ${BORDER}` }}>
                ...
                <textarea
                  ...
                  style={{ ...darkInput, resize: 'none', opacity: busy ? 0.5 : 1 }}
                />
                <div style={{ fontFamily: FONT_BODY, fontSize: FS_CAPTION, color: DIM, marginTop: '0.3125rem' }}>Ctrl + Enter to generate</div>
              </div>

              {/* Last 5 Prompts */}
              <div style={{ padding: '0.875rem 1.125rem', borderBottom: `1px solid ${BORDER}` }}>
                <div style={{ fontFamily: FONT_BODY, fontSize: FS_OVERLINE, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(200,170,80,0.5)', marginBottom: '0.5rem' }}>
                  Last 5 Prompts
                </div>
                {promptHistory.length === 0 ? (
                  <div style={{ fontFamily: FONT_BODY, fontSize: FS_CAPTION, color: DIM, fontStyle: 'italic' }}>No prompts yet.</div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                    {promptHistory.map((p, i) => (
                      <button
                        ...
                        disabled={busy}
                        title={p}
                        className="hover:border-[color:var(--hud-border-hi)] hover:text-[color:var(--hud-text)]"
                        style={{ background: 'transparent', border: `1px solid ${BORDER}`, color: DIM, fontFamily: FONT_BODY, fontSize: FS_CAPTION, padding: '0.3125rem 0.625rem', borderRadius: RADIUS.sm, cursor: 'pointer', textAlign: 'left', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                      >{p}</button>
                    ))}
                  </div>
                )}
              ...
              <div style={{ padding: '0.875rem 1.125rem 1.125rem', borderTop: `1px solid ${BORDER}` }}>
                <div style={{ fontFamily: FONT_BODY, fontSize: FS_OVERLINE, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(200,170,80,0.5)', marginBottom: '0.5rem' }}>
                  Reference Assets
                </div>
                <div style={{ fontFamily: FONT_BODY, fontSize: FS_CAPTION, color: DIM, marginBottom: '0.625rem', lineHeight: 1.5 }}>
                ...
                <button
                  ...
                  className="hover:border-[color:var(--hud-gold)]"
                  style={{ width: '100%', padding: '0.625rem', border: `1px dashed ${freshUploading ? BORDER : BORDER_HI}`, borderRadius: RADIUS.md, background: 'transparent', cursor: freshUploading ? 'wait' : 'pointer', fontFamily: FONT_BODY, fontSize: FS_CAPTION, color: freshUploading ? DIM : TEXT, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}
                >
                  {freshUploading
                    ? <><div style={{ width: '0.75rem', height: '0.75rem', borderRadius: RADIUS.full, border: `1.5px solid ${HUD.gold}`, borderTopColor: 'transparent', animation: 'mf-spin 0.8s linear infinite' }} />Uploading…</>
                ...
                {freshAssets.length > 0 && (
                  <div style={{ fontFamily: FONT_BODY, fontSize: FS_CAPTION, color: DIM, marginTop: '0.375rem', textAlign: 'center' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {/* ── Progress dialog ──────────────────────────── */}
          {dialogOpen && (
            <div style={{ position: 'fixed', inset: 0, zIndex: 'var(--z-hud-overlay)' as unknown as number, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(4px)' }}>
              <div style={{ width: '22.5rem', background: PANEL_BG, border: `1px solid ${BORDER_HI}`, borderRadius: RADIUS.lg, boxShadow: '0 24px 64px rgba(0,0,0,0.8)', overflow: 'hidden' }}>
                <div style={{ padding: '0.875rem 1.125rem', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                  <div style={{ width: '0.875rem', height: '0.875rem', borderRadius: RADIUS.full, border: `2px solid ${HUD.gold}`, borderTopColor: 'transparent', animation: 'mf-spin 0.8s linear infinite', flexShrink: 0 }} />
                  <span style={{ fontFamily: FONT_BODY, fontSize: FS_LABEL, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: HUD.gold }}>
                    {saving ? 'Saving Map' : editModeActive ? 'Applying Edit' : 'Generating Map'}
                  </span>
                  <span style={{ marginLeft: 'auto', fontFamily: FONT_BODY, fontSize: FS_CAPTION, color: DIM, fontVariantNumeric: 'tabular-nums' }}>{elapsed}s</span>
                </div>
                  ...
                  {STEPS.map((s, i) => {
                    ...
                    return (
                      <div key={s} style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                        <div style={{ width: '0.5rem', height: '0.5rem', borderRadius: RADIUS.full, flexShrink: 0, background: done ? HUD.gold : active ? 'var(--state-success)' : 'transparent', border: `1.5px solid ${done ? HUD.gold : active ? 'var(--state-success)' : DIM}`, boxShadow: active ? '0 0 8px #4EC87A88' : 'none', animation: active ? 'mf-pulse 1.2s ease-in-out infinite' : 'none' }} />
                        <span style={{ fontFamily: FONT_BODY, fontSize: FS_SM, color: done ? TEXT : active ? 'var(--state-success)' : DIM }}>{s}</span>
                        {done && <span style={{ marginLeft: 'auto', fontFamily: FONT_BODY, fontSize: FS_CAPTION, color: HUD.gold }}>✓</span>}
                      </div>
                    )
                  })}
                </div>
                {log.length > 0 && (
                  <div style={{ margin: '0 0.875rem 0.875rem', padding: '0.5rem 0.625rem', background: 'rgba(0,0,0,0.4)', border: `1px solid ${BORDER}`, borderRadius: RADIUS.md, display: 'flex', flexDirection: 'column', gap: '0.1875rem' }}>
                    {log.map((entry, i) => (
                      <div key={i} style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-overline)', color: entry.startsWith('Error') ? 'var(--state-failure)' : i === log.length - 1 ? TEXT : DIM, animation: i === log.length - 1 ? 'mf-pulse 1.2s ease-in-out 2' : 'none' }}>
                        {'> '}{entry}
                      </div>
                    ))}
                  </div>
          ...
          {/* Error */}
          {error && (
            <div style={{ flexShrink: 0, padding: '0.625rem var(--space-5)', background: 'rgba(224,80,80,0.08)', borderBottom: `1px solid rgba(224,80,80,0.2)`, fontFamily: FONT_BODY, fontSize: FS_LABEL, color: 'var(--state-failure)' }}>
              {error}
            </div>
          )}
          ...
          {/* Success toast */}
          {savedMsg && (
            <div style={{ flexShrink: 0, padding: '0.625rem var(--space-5)', background: 'rgba(78,200,122,0.08)', borderBottom: `1px solid rgba(78,200,122,0.2)`, fontFamily: FONT_BODY, fontSize: FS_LABEL, color: 'var(--state-success)', display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
              ✓ {savedMsg}
          ...
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative' }}>
            {imageUrl ? (
              ...
              <div style={{ textAlign: 'center', pointerEvents: 'none' }}>
                <div style={{ fontFamily: FONT_BODY, fontSize: FS_H3, color: 'rgba(200,170,80,0.12)', letterSpacing: '0.3em', textTransform: 'uppercase', marginBottom: '0.5rem' }}>Map Forge</div>
                <div style={{ fontFamily: FONT_BODY, fontSize: FS_SM, color: DIM }}>Enter a mission brief and hit Generate</div>
              </div>
            )}
          ...
          {/* Save / action bar */}
          {imageUrl && !busy && (
            <div style={{ flexShrink: 0, padding: '0.75rem var(--space-5)', borderTop: `1px solid ${BORDER}`, background: PANEL_BG, display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
              <input
                value={mapName}
                onChange={e => setMapName(e.target.value)}
                placeholder="Map name…"
                style={{ ...darkInput, maxWidth: '17.5rem' }}
              />
              <button
